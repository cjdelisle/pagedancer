...
var compileFile = function (fileName, builder, tempDir, callback)
{
    ...
    nThen(function (waitFor) {
        ...
        // compute the compiler cmd early so that it can be altered in the file.
        var compilerArgs = ['-c','-x','cpp-output','-o',outFile];
        compilerArgs.push.apply(compilerArgs, getFlags(state, fileName, false));
        if (state.useTempFiles) {
            compilerArgs.push(preprocessed);
        } else {
            compilerArgs.push('-');
        }
        fileObj.compilerArgs = compilerArgs;
        ...
        //debug("CC");
        cc(state.gcc, fileObj.compilerArgs, waitFor(function (err) {
            if (err) { throw err; }
            fileObj.obj = outFile;
        }), fileContent);
